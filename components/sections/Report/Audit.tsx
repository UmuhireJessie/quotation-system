...
const Audit = () => {
    ...
    const handleSendEmailReport = (e:any) => {
        e.preventDefault();
        handleExportAndSendEmail(filteredData, reportEmail, 'logs')
        setReportEmail('')
        setOpenSendEmailModal(false)
    }
    ...
    const filteredData = data?.filter((row: any) => {
        ...
        const hostNameTypeMatch = row.hostName
            .toLowerCase()
            .includes(filters.hostName.toLowerCase());

        const ipAddressMatch = row.ipAddress
            .toLowerCase()
            .includes(filters.ipAddress.toLowerCase());

        const userNameMatch = row.sName
            .toLowerCase()
            .includes(filters.username.toLowerCase()) || row.fName
                .toLowerCase()
            ...
            <div className="mt-[7rem] ml-[18rem] mr-7 mb-4 bg-white p-6 rounded-md">
                ...
                <div className='flex justify-end'>
                    ...
                    <button
                        ...
                        onClick={(e) => {
                            e.preventDefault();
                            handleExportData(filteredData, 'logs')
                        }}
                    >
                ...
                <Table  ref={tableRef} className="mb-10 text-[#06091b]">
                    <Thead className="">
                        <Tr>
                            <Th className="text-start text-[14px] py-6 border-b border-[#e0e0e0] pl-4">Date & Time</Th>
                            <Th className="text-start text-[14px] py-6 border-b border-[#e0e0e0]">IP Address</Th>
                            <Th className="text-start text-[14px] py-6 border-b border-[#e0e0e0]">Host Name</Th>
                            <Th className="text-start text-[14px] py-6 border-b border-[#e0e0e0]">Description</Th>
                            <Th className="text-start text-[14px] py-6 border-b border-[#e0e0e0]">User name</Th>
                        ...
                        <Tr className="border-b border-[#888787]">
                            ...
                            <Td className="py-4">
                                <input
                                    ...
                                    onChange={(e) => handleFilterChange(e, 'ipAddress')}
                                />
                            </Td>
                            <Td className="py-4">
                                <input
                                    ...
                                    onChange={(e) => handleFilterChange(e, 'hostName')}
                                />
                            </Td>
                            <Td className="py-4">
                                <input
                                ...
                                currentRows?.map((row: any, index: any) => {
                                    return (
                                        <Tr key={index} style={{ backgroundColor: index % 2 === 0 ? '#f7f7f7' : '#ffffff' }}>
                                            ...
                                            <Td className="py-[10px] border-b border-[#e6e6e6] text-sm">{row.fName} {row.sName}</Td>
